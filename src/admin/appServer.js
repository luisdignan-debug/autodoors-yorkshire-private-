...
function dashboardPage(leads, config, supplierEmails = [], store = { state: { leads, supplierEmails } }) {
  ...
  return pageShell(
    ...
    <details class="drawer"><summary>More dashboard detail and action queues</summary><div class="drawer-body">
...
function pageShell(title, body, status = 200) {
  ...
  const mobileBottomNav = `<nav class="mobile-bottom-nav">
    ...
    <a href="#nav-menu" class="bnav-item"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16"></path><path d="M4 12h16"></path><path d="M4 17h16"></path></svg><span>Menu</span></a>
  </nav>`;
...
(function(){const p=location.pathname;document.querySelectorAll('.bnav-item[href]').forEach(a=>{const h=a.getAttribute('href');if(h&&h!=='#nav-menu'&&(p===h||p.startsWith(h+'/'))){a.classList.add('bnav-active');}});})();
...
function workflowBadge(value) {
  return `<span class="badge grey">${escapeHtml(workflowLabel(value))}</span>`;
}
...
function ragBadge(value) {
  ...
  return `<span class="badge ${escapeAttr(value || "green")}"><span class="risk-dot ${escapeAttr(value || "green")}"></span>${label}</span>`;
}
